...
#新增小时统计特征：'windlevel_max', 'windlevel_mean','humi_max','humi_mean', 'dswrf_max', 'dswrf_mean'
    ...
    data = df.loc[:, ['aqi','co','no2','so2','o3_8h','pm2_5','pm10','pointname','time','longitude', 'latitude','wda_max', 'wda_mean', 'wda_min', 'temp_max', 'temp_mean', 'temp_min', 'dswrf_max', 'dswrf_mean', 'dswrf_min', 'humi_max', 'humi_mean', 'humi_min']]
    # 原特征
...
def train():
    ...
    data = pd.read_csv('./data/广东站点气象/merge.csv')
    ...
    model.fit(X, Y)
    pickle.dump(model, open('LGBboost_cli_pointaqi_Regressor' + str(future) + '.dat', 'wb'))
...
def test():
    ...
    for point in point_dic.keys():
        ...
        if np.array(Y).mean() <=10:
            anomaly_point.append(point)
            continue
        # X.extend(train_X)
        # Y.extend(train_Y)
        loaded_model = pickle.load(open('LGBboost_cli_pointaqi_Regressor' + str(future) + '.dat', 'rb'))
    ...
    res.to_csv('LGBboost_cli_pointaqi.csv')
def merge_data():
    import glob
    import os
    import pandas as pd
    inputfile = str(os.path.dirname(os.getcwd()))+"/站点预测/data/广东站点气象/*.csv"
    outputfile =  str(os.path.dirname(os.getcwd()))+"/站点预测/data/广东站点气象/merge.csv"
